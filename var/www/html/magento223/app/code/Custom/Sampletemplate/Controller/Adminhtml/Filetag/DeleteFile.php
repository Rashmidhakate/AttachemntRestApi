...
class DeleteFile extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        \Custom\Sampletemplate\Model\Filetag $attachModel,
    ...
    {
        return $this->_authorization->isAllowed('Custom_Sampletemplate::delete');
    }
    ...
    public function execute()
    {
        ...
        if ($id) {
            try {
                ...
                $model->load($id);
                $currentFile = $model->getFiletagLogo();
                $mediaDirectory = $this->fileSystem
                    ->getDirectoryRead(\Magento\Framework\App\Filesystem\DirectoryList::MEDIA);
                $fileRootDir = $mediaDirectory->getAbsolutePath().'filetag/images/';
                if ($this->file->isExists($fileRootDir . $currentFile)) {
                    $this->file->deleteFile($fileRootDir . $currentFile);
                    $model->setFiletagLogo('');
                    $model->save();
                    $this->messageManager->addSuccess(__('The file has been deleted.'));
                }
                return $resultRedirect->setPath('sampletemplate/filetag/edit', ['filetag_id' => $id]);
            } catch (\Exception $e) {
